...
<p class="fs-2 text-center">ONGOING TRANSACTIONS</p><hr>
...
<table id="myTable" class="table table-responsive table-hover table-bordered" style="border-color:#043047;">
    ...
    <tbody>
        <?php
        $count = 0;
        $fetchdata = $database->getReference('TransactionOngoing')->getValue();
        if(is_array($fetchdata)){
          foreach($fetchdata as $key => $row){
            $fetchdata2 = $database->getReference('TransactionOngoing/'.$key)->getValue();
            if(is_array($fetchdata2)){
              foreach($fetchdata2 as $key2 => $row2){
                $fetchdata3 = $database->getReference('TransactionOngoing/'.$key.'/'.$key2)->getValue();
