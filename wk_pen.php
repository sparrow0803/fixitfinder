...
if(isset($_POST['verify'])){
    ...
    $houpdate = $database->getReference('worker_users/'.$_POST['verify'])->update($updatedata);
    ...
    <p style="font-size: 15px;">Your FixitFinder worker account is now verified!</p>
    <p style="font-size: 15px;">You can now access the features of the worker interface.</p><br>
...
if(isset($_POST['send'])){
    ...
    $deletequery_result = $database->getReference('worker_users/'.$_POST['wid'])->remove();

    $fetch1 = $database->getReference('workers_selfie/'.$_POST['wid'])->getValue();
    if (is_array($fetch1)){
      $database->getReference('workers_selfie/'.$_POST['wid'])->remove();
    }

    $fetch2 = $database->getReference('workers_id/'.$_POST['wid'])->getValue();
    if (is_array($fetch2)){
      $database->getReference('workers_id/'.$_POST['wid'])->remove();
    }

    $fetch3 = $database->getReference('workers_police_clearance/'.$_POST['wid'])->getValue();
    if (is_array($fetch3)){
      $database->getReference('workers_police_clearance/'.$_POST['wid'])->remove();
    }

    $updateData = [
      'worker_verification'=>'In Process'
    ];
    $updatequery_result = $database->getReference('worker_users/'.$_POST['wid'])->update($updateData);
    ...
    <p style="font-size: 15px;">Good day, user! We regret to inform you that your worker account did not meet the requirements for validation due to the following:</p>
    '.$op.'
    <p style="font-size: 15px;">Please comply all the following queries by resubmitting another form request through the application.</p><br>
    <p style="font-size: 13px">© FixitFinder</p>
    </div>
    ...
    if($mail->send() && $deletequery_result && $updatequery_result){
...
<head>
    ...
    <title>Workers Pending Verification</title>
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="verifymodalLabel">Worker Information</h1>
...
<p class="fs-2 text-center">WORKERS PENDING VERIFICATION</p><hr>
        ...
        if(is_array($fetchdata) && is_array($fetchdata2)){
            foreach($fetchdata as $key => $row){
              foreach($fetchdata2 as $key2 => $row2){
                if($row2['worker_verification'] != 'Pending' || $key2 != $key){
                    continue ;
                }
                else{
                    $count += 1;
               ...
               <tr>
                    ...
                    <td scope="col" class="text-center">
                        <div class='btn-group' role='group' aria-label='Basic mixed styles example'>
                            <button type='button' id='<?php echo $key2; ?>' class='btn btn-outline-success verifybtn' value='<?php echo $key2; ?>'><i class="bi bi-check-lg"></i></button>
                        </div>
                    </td>
               </tr> 
        <?php
...
<script>
    ...
    text: "Email Sent Succesfully! Wait for the resubmission form of the user.",
    icon: "success",
    confirmButtonText: 'OK',
    confirmButtonColor: "#043047",
    });
