...
if(isset($_POST['send'])){
  ...
  $database->getReference('WK_Warning/'.$_POST['id'])->push($postData);
  $database->getReference('WK_Suspension/'.$_POST['id'])->push($postData2);

  $updateData = [
    'worker_verification'=>'Suspended',
    'worker_last_update'=>date('Y-m-d'),
    'lift_suspension'=>date('Y-m-d', strtotime($now. '+30 days'))
  ];

  $database->getReference('worker_users/'.$_POST['id'])->update($updateData);
  ...
  <p style="font-size: 15px;">We noticed that your worker account has been reported by our users and violated our Terms and Conditions which includes the following:</p>
  '.$op.'
  <p style="font-size: 15px;">We also noticed that your worker account has a record of three consecutive warnings which resulted to your account suspension.</p>
  <p style="font-size: 15px;">Please wait for 30 days to re-claim your worker account.</p><br>
  ...
  if($mail->send()){
    $_SESSION['email_success'] = "This worker account will be suspended for 30 days";
  }
  else{
    ...
    <p style="font-size: 15px;">We noticed that your worker account has been reported by our users and violated our Terms and Conditions which includes the following:</p>
    '.$op.'
    <p style="font-size: 15px;">We also noticed that your worker account has a record of three consecutive warning which resulted to your account suspension.</p>
    <p style="font-size: 15px;">Please wait for 60 days to re-claim your worker account.</p><br>
    <p style="font-size: 13px">© FixitFinder</p>
    </div>
    ...
    if($mail->send()){
      $_SESSION['email_success'] = "This worker account will be suspended for 60 days";
    }
    else{
      $_SESSION['email_fail'] = "FAIL";
    }
    ...
    $database->getReference('users/'.$_POST['id'])->update($updateData2);
    ...
    <h2 style="font-size: 18px;"> ACCOUNT SUSPENSION! </h2>
    <p style="font-size: 15px;">Your worker account has been reported by our users and violated our Terms and Conditions which results to the suspension of your account due to the following:</p>
    '.$op.'
    <p style="font-size: 15px;">Your worker account is now in the 3rd suspension state.</p><br>
    <p style="font-size: 13px">© FixitFinder</p>
    </div>
    ...
    if($mail2->send()){
      ...
      <p style="font-size: 15px;">After thorough investigation, we noticed that your worker account has been reported by our users and violated our Terms and Conditions which includes the following:</p>
      '.$op.'
      <p style="font-size: 15px;">Please take note that after three warnings, your worker account will be up for suspension.</p><br>
      <p style="font-size: 13px">© FixitFinder</p>
      </div>
      ...
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div class="collapse navbar-collapse flex-grow-0" id="navbarSupportedContent">
          <ul class="navbar-nav nav-fill nav-pills nav-tabs">
            ...
            <li class="nav-item dropdown">
              <a class="nav-link active dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false" style="color: white;"><i class="bi bi-wrench-adjustable me-2"></i>Workers</a>    
                <ul class="dropdown-menu">
...
<div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="infomodalLabel">Worker Information</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
...
<p class="fs-2 text-center">VERIFIED WORKERS</p><hr>
    ...
    <tbody>
        ...
        ?>
               <tr>
                    ...
                    <td scope="col" class="text-center"><?= $row2['worker_last_update']; ?></td>
                    <td scope="col" class="text-center"><?= $row2['worker_verification']; ?></td>
                    <td scope="col" class="text-center">
                        <div class='btn-group' role='group' aria-label='Basic mixed styles example'>
                            <button type='button' id='<?php echo $key2; ?>' class='btn btn-outline-secondary infobtn' value='<?php echo $key2; ?>'><i class="bi bi-info-circle"></i></button>
                            <button type='button' id='<?php echo $key2; ?>' class='btn btn-outline-warning warningbtn' value='<?php echo $key2; ?>'><i class="bi bi-exclamation-circle"></i></button>
                            <button type='button' id='<?php echo $key2; ?>' class='btn btn-outline-danger suspendbtn' value='<?php echo $key2; ?>'><i class="bi bi-dash-circle"></i></button>
                        </div>
                    </td>
               </tr> 
        <?php
